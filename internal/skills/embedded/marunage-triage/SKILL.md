...
```json
{"external_id": "<id>", "decision": "task" | "skip", "reason": "<1文>", "title": "<改善タイトル>", "priority": 0}
```

- `decision` — `task` または `skip` のいずれか。それ以外の値は dispatcher が拒絶する
- `reason` — 上記「判定ロジック」で適用したルール番号と根拠
  例: `"rule 1: @me が直接メンションされている"`
- `title` — 任意。`task` 判定のときに、source プラグインが入れた
  raw タイトル（Slack 本文の先頭行など）を「**対象 + 動詞**」形式の
  改善タイトルで上書きする。Web UI のタスク一覧でタイトル単体から
  「何にどう対応する計画か」が一目で判断できるようにするのが目的。
  - 良い例: `"佐藤さんに来週MTG日程を返信"` / `"PR #123 をレビュー"` /
    `"ステージング DB 増強 Issue 起票"`
  - 悪い例: `"来週のMTGについて確認したい件"`（誰が何をするか不明）
  - 80 文字以内・1 行・記号や改行を含めない
  - 適切な改善が思いつかない場合はフィールド自体を省略すれば raw が残る
  - `skip` 判定で指定しても無視される
...
```json
{"external_id": "T123.456", "decision": "task", "reason": "rule 1: @me が直接メンションされている", "title": "佐藤さんに来週MTG日程を返信", "priority": 1}
```
