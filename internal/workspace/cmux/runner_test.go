...
// ExecRunner against /bin/sh so the package keeps a smoke test against
// a binary that always exists on darwin / linux. We do not invoke cmux
// itself — that would require the user's cmux to be installed and
// versioned, which CI cannot guarantee.
...
// exec.ErrNotFound, so client.NewWorkspace can map it to
// ErrCmuxNotFound via errors.Is. The fixed-name "marunage-no-such-tool"
...
func TestExecRunnerSurfacesMissingBinaryAsExecErrNotFound(t *testing.T) {
	r := cmux.ExecRunner{}
	_, _, err := r.Run(context.Background(), "marunage-no-such-tool-xyzzy")
	if err == nil {
